...
class MegaRastreoApiWeb {
    ...
    async startAutoUpdate(imeis, onFlushCallback = null) {
        ...
        // Accumulate IMEIs instead of replacing
        const currentSet = new Set(this.subscribedImeis || []);
        let hasNewImeis = false;
        
        for (const imei of imeis) {
            if (!currentSet.has(imei)) {
                currentSet.add(imei);
                hasNewImeis = true;
            }
        }
        ...
        if (hasNewImeis) {
            this.subscribedImeis = Array.from(currentSet);
            // If already connected, just update the subscription
            if (this.socket && this.socket.connected) {
                this.socket.emit("login_client", this.subscribedImeis);
                logger.info(`🔌 Updated MegaRastreo stream. Total listening: ${this.subscribedImeis.length}`);
                return;
            }
        }

        if (this.socket) return; // Connection already initiating/active
        
        const agent = new https.Agent({ servername: Url.MegarastreoBase });
    ...
    addImeis(newImeis) {
        if (!newImeis || !Array.isArray(newImeis)) return;
        ...
        if (added) {
            ...
            if (this.socket && this.socket.connected) {
                this.socket.emit("login_client", this.subscribedImeis);
                logger.info(`🔌 Added IMEIs to MegaRastreo stream. Total listening: ${this.subscribedImeis.length}`);
            }
        }
    }
